...
public final class ReadLikelihoods<A extends Allele> implements SampleList, AlleleList<A> {
    ...
    public void addMissingAlleles(final Collection<A> candidateAlleles, final double defaultLikelihood) {
        ...
        final A[] newAlleles = (A[]) ListUtils.union(alleles.asListOfAlleles(), allelesToAdd).toArray();
    ...
    public void filterPoorlyModeledReads(final double maximumErrorPerBase) {
        ...
        IntStream.range(0, samples.numberOfSamples()).forEach(s -> {
            final GATKRead[] sampleReads = readsBySampleIndex[s];
            final int[] removeIndices = IntStream.range(0, sampleReads.length)
                    .filter(r -> readIsPoorlyModelled(s, r, sampleReads[r], maximumErrorPerBase)).toArray();
    ...
    public void contaminationDownsampling(final Map<String, Double> perSampleDownsamplingFraction) {
        ...
        for (int s = 0; s < samples.numberOfSamples(); s++) {
            ...
            if (fraction >= 1.0) {
                final int[] removeIndices = IntStream.range(0, readsBySampleIndex[s].length).toArray();
    ...
    public void filterToOnlyOverlappingUnclippedReads(final SimpleInterval location) {
        ...
        for (int s = 0; s < sampleCount; s++) {
            final GATKRead[] sampleReads = readsBySampleIndex[s];
            final int[] removeIndices = IntStream.range(0, sampleReads.length)
                    .filter(r -> !unclippedReadOverlapsRegion(sampleReads[r], locContig, locStart, locEnd)).toArray();
            removeSampleReads(s,removeIndices,alleleCount);
        }
    }
    ...
    private void removeSampleReads(final int sampleIndex, final int[] removeIndices, final int alleleCount) {
        ...
        if (indexByRead != null) {
            Arrays.stream(removeIndices).forEach(n -> indexByRead.remove(sampleReads[n]));
        }
        final boolean[] removeIndex = new boolean[sampleReadCount];
        final int firstDeleted = removeIndices[0];
        Arrays.stream(removeIndices).forEach(n -> removeIndex[n] = true);

        final int newSampleReadCount = sampleReadCount - removeIndices.length;
    ...
    public PerReadAlleleLikelihoodMap toPerReadAlleleLikelihoodMap(final int sampleIndex) {
        ...
        for (int a = 0; a < alleles.numberOfAlleles(); a++) {
            ...
            for (int r = 0; r < sampleReads.length; r++)
