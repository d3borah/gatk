...
public final class GATKVariantContextUtils {
    ...
    public static GenotypesContext subsetAlleles(final VariantContext vc,
        ...
        // from a subset of alternate alleles to use, determine the likelihood indexes to use for each genotype
        final List<List<Integer>> genpotypesLikelihoodIndexesToUse = determineGenotypesLikelihoodIndexesToUse(vc, allelesToUse);

        // create the new genotypes
        return createGenotypesWithSubsettedLikelihoods(vc.getGenotypes(), vc, allelesToUse, genpotypesLikelihoodIndexesToUse, assignGenotypes);
    }
     ...
     * @param genpotypesLikelihoodIndexesToUse   for each genotype, the indexes in the PL to use given the allelesToUse (@see #determineGenotypesLikelihoodIndexesToUse())
     * @param assignGenotypes          assignment strategy for the (subsetted) PLs
    ...
    private static GenotypesContext createGenotypesWithSubsettedLikelihoods(final GenotypesContext originalGs,
                                                                            ...
                                                                            final List<Allele> allelesToUse,
                                                                            final List<List<Integer>> genpotypesLikelihoodIndexesToUse,
                                                                            final GenotypeAssignmentMethod assignGenotypes) {
        ...
        for ( int k = 0; k < originalGs.size(); k++ ) {
            ...
            if ( !g.hasLikelihoods() ) {
                ...
                if ( genpotypesLikelihoodIndexesToUse == null ) {
                    ...
                    newLikelihoods = new double[genpotypesLikelihoodIndexesToUse.get(k).size()];
                    int newIndex = 0;
                    for ( final int oldIndex : genpotypesLikelihoodIndexesToUse.get(k) )
    ...
    private static GenotypesContext fixGenotypesFromSubsettedAlleles(final GenotypesContext originalGs, final VariantContext originalVC, final List<Allele> allelesToUse) {
        ...
        final List<List<Integer>> genpotypesLikelihoodIndexesToUse = determineGenotypesLikelihoodIndexesToUse(originalVC, allelesToUse);

        // create the new genotypes
        return createGenotypesWithSubsettedLikelihoods(originalGs, originalVC, allelesToUse, genpotypesLikelihoodIndexesToUse, GenotypeAssignmentMethod.DO_NOT_ASSIGN_GENOTYPES);
    }
    /**
     * Determine which likelihood indexes to use for each genotype for a subset of alleles
     *
     * @param originalVC        the original VariantContext
     * @param allelesToUse      the subset of alleles to use
     * @return likelihood indexes for each genotype
     */
    private static List<List<Integer>> determineGenotypesLikelihoodIndexesToUse(final VariantContext originalVC, final List<Allele> allelesToUse) {
    ...
    private static List<List<Integer>> getLikelihoodIndexes(final VariantContext originalVC, final boolean[] alleleIndexesToUse) {
        ...
        final List<List<Integer>> likelihoodIndexesPerGenotype = new ArrayList<>(10);
        ...
        for (final Genotype g : originalVC.getGenotypes()) {
            ...
            for ( int PLindex = 0; PLindex < numLikelihoods; PLindex++ ) {
                ...
                if ( GenotypeLikelihoods.getAlleles(PLindex, g.getPloidy()).stream().allMatch(i -> alleleIndexesToUse[i]) )
